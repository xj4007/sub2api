...
| 节点名 | 服务器 | 公网 IP | WG IP | WG 监听端口 |
|---|---|---|---|---|
| `db1` | 服务器1 | `154.201.64.184` | `10.77.0.1` | `51820` |
| `db2` | 服务器2 | `154.12.80.55` | `10.77.0.2` | `51820` |
| `db3` | 服务器3 | `156.225.18.73` | `10.77.0.3` | `51820` |
| `db4` | 服务器7 | `211.101.237.129` | `10.77.0.7` | `51820` |
| `db5` | 服务器8 | `101.237.129.94` | `10.77.0.8` | `51820` |
| `app4` | 服务器4 | `154.12.21.52` | `10.77.0.4` | `51820` |
| `app5` | 服务器5 | `45.192.105.162` | `10.77.0.5` | `51820` |
| `app6` | 服务器6 | `156.225.20.29` | `10.77.0.6` | `51820` |
| `app7` | 服务器9 | `222.186.180.129` | `10.77.0.9` | `51821` |
| `app8` | 服务器10 | `171.80.2.160` | `10.77.0.10` | `51820` |
...
| 服务器9 | `222.186.180.129:39201` | `sub2api` + 本地 `pgproxy` |
| 服务器10 | `171.80.2.160` | `sub2api` + 本地 `pgproxy` |
...
| 服务器9 | `222.186.180.129` | `39201` | `root` | `XIANjian4SANyun` |
| 服务器10 | `171.80.2.160` | `22` | `root` | `XIANjian4SANyun` |
...
### 16.3.2 app7 扩容时的特殊情况（2026-04-17）

本次新增第 4 台业务机：

- `app7 / 222.186.180.129 / 10.77.0.9`

实际部署时发现：

- `app7` 使用默认 WireGuard 监听端口 `51820` 时，出站发包正常，但几乎没有 peer 回包
- 现象表现为：
  - `wg show` 在 `app7` 上几乎只有 sent、没有 received
  - `10.77.0.x` 的 PostgreSQL / Redis Sentinel 端口对 `app7` 超时
  - 应用层启动时表现为数据库 / Sentinel 连接异常

最终确认：

> `app7` 的 `51820/udp` 入站存在异常，而不是整个 WireGuard 配置错误。

本次修复方式：

- 仅对 `app7` 改用新的 WireGuard 监听端口：`51821`
- 其余 8 台节点保持 `51820`
- 所有节点指向 `app7` 的 peer endpoint 改成：

```text
222.186.180.129:51821
```

修复后结果：

- `app7` 与各节点握手恢复
- `10.77.0.1/.2/.3/.7/.8` 的 PostgreSQL / Sentinel 端口恢复可达
- `app7` 的 `sub2api` 正常健康
- 管理员登录返回 `200`

额外说明：

- 在排查阶段，曾临时让 `app7` 的 `pgproxy` 不包含 `db1`，并把 Sentinel seed 收窄到可达集合
- 随着 WireGuard 链路恢复，`app7` 当前已经重新恢复到标准形态：
  - `pgproxy` 已重新包含 `db1~db5`
  - Sentinel seed 已恢复包含 `10.77.0.1/.2/.3/.7/.8`

所以当前正确口径是：

> `app7` 的长期特殊项只有 **WG 监听端口 = 51821**，应用层后端配置不再需要单独偏离其它业务机。

### 16.3.3 app8 扩容结果（2026-04-17）

本次新增第 5 台业务机：

- `app8 / 171.80.2.160 / 10.77.0.10`

本次接入结果：

- `app8` 未复现 `app7` 的 `51820/udp` 入站异常
- 继续沿用默认 WireGuard 监听端口：`51820`
- `app8` 到 `10.77.0.1/.2/.3/.7/.8` 的 PostgreSQL / Patroni / Sentinel 端口一次性全部可达
- `app8` 应用层直接按标准配置启动成功，无需像 `app7` 那样先做后端收敛

应用层最终验证结果：

- `/health` 返回 `ok`
- 容器状态 `healthy`
- 运行镜像：`sub2api:ha-20260417c`
- 运行版本：`0.1.113`
- 管理员登录返回 `200`

所以当前正确口径是：

> `app8` 没有额外特殊项，保持和 `app4/app5/app6` 一样的标准业务机配置即可。

### 16.3.4 关于 Redis/Patroni 跨节点抖动的现场说明

在扩容 `app8` 的同时，现场继续观察到数据层存在轻微跨节点抖动，主要体现在：

- Sentinel 偶尔将 `10.77.0.1:26379`、`10.77.0.2:26379`、`10.77.0.7:26379` 标记为短暂 `sdown`
- Redis replica 偶尔将 `10.77.0.1:6379`、`10.77.0.2:6379`、`10.77.0.7:6379` 标记为短暂 `sdown`

但在 `app8` 的实际接入过程中，这些端口仍然可以连通，最终应用层验证全部通过：

- `10.77.0.1/.2/.3/.7/.8:5432` 可达
- `10.77.0.1/.2/.3/.7/.8:8008` 可达
- `10.77.0.1/.2/.3/.7/.8:26379` 可达
- `sub2api` 健康、版本正确、管理员登录返回 `200`

因此当前结论是：

> 数据层存在轻微的跨节点网络抖动告警，但不影响 `app8` 作为标准业务节点接入现网。
